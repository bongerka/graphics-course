...
void castRay(in vec3 direction, in vec3 origin, out vec3 hitPoint, out bool rayDidHit) {
    for (int fake_iters = 100; fake_iters > 0; fake_iters--) {
...
void simulate_planet_rotation() {
    ...
    bodies[1].center = vec3(((params.iMousePosition - iResolution / 2.0) / min(iResolution.x, iResolution.y)) * vec2(10.0, -10.0), bodies[0].center.z-0.5);
